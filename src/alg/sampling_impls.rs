...
impl<const ARITY: usize, V: std::hash::Hash + Eq + Clone + Copy> DynamicClustering<ARITY, V> {
    #[inline(always)]
    pub fn f_b(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> Float {
        // f_b = sigma* size + (sigma * vol)/deg(x^*)

        let size = Float::from(self.tree_data.size[node_idx] as f64);
        let vol: ordered_float::OrderedFloat<f64> = self.tree_data.volume[node_idx].0;
    ...
    #[inline(always)]
    pub fn f_delta_read(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> FDelta {
    ...
    #[inline(always)]
    pub fn f(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> Contribution {
        // f_s = f_b - f_delta

        let f_b = self.f_b(node_idx, info);
        let f_delta = self.f_delta_read(node_idx, info);
        Contribution(Real::max(f_b - f_delta.0, Float::from(0.0)))
    }
    ...
    #[inline(always)]
    pub fn h_b(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> HB {
        let saved_timestamp = self.tree_data.timestamp[node_idx];
        let cur_timestamp = info.timestamp;
    ...
    #[inline(always)]
    pub fn h_s(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> HS {
        let saved_timestamp = self.tree_data.timestamp[node_idx];
        let cur_timestamp = info.timestamp;
    ...
    #[inline(always)]
    pub fn g(&self, node_idx: TreeIndex, info: &SamplingInfo<V>) -> SmoothedContribution {
    ...
    fn sample_impl(
        ...
        fill: impl Fn(&Self, &mut [Float; ARITY], TreeIndex, &SamplingInfo<V>) -> usize,
